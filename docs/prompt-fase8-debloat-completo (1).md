...
1. Criar `AiFeatureScanner.java` (Copilot, Recall, Click to Do, Cocreator, Copilot no Edge) conforme a tabela da seção 1. **Importante:** para cada item que suportar as duas ações (Desativar e Desinstalar), implemente ambas — não escolha só uma. Onde a desinstalação não for tecnicamente possível (ex: Click to Do, Cocreator, Copilot no Edge), implemente só a desativação e documente a limitação na descrição do item, exatamente como consta na tabela do documento.
2. Criar `ConsumerFeatureScanner.java` (anúncios, sugestões, pesquisa Bing, notificações) conforme a tabela da seção 2.
3. Expandir `ActionExecutor` com os métodos de ação para os novos itens — para os itens de IA com as duas opções, criar os pares `disableAiFeature`/`uninstallAiFeature` (e seus respectivos `restore`), seguindo exatamente o mesmo padrão de segurança já usado (checar `LockManager.isLocked` → `BackupManager.snapshotBeforeAction` → aplicar → registrar em `actions_history`) — **nenhuma exceção a essa regra**, mesmo para desinstalação.
3.1. Na `ItemDetailView`, exibir os dois botões separados (Desativar / Desinstalar) para os itens que suportam ambos, com o botão de desinstalar visualmente diferenciado (mais "sério", já que é ação mais permanente).
